		...
		def amend_db_name(self):
			...
			for db in cad_db :
				for db_bak in cad_bak_db :
					operation="sed -i s/%s/%s/g %s" %(db,db_bak,cad_file)	
					commands.getoutput(operation)

			for db in pcw_db :
				for db_bak in pcw_bak_db :
					operation="sed -i s/%s/%s/g %s" %(db,db_bak,pcw_file)	
					commands.getoutput(operation)
...
class MultiThread (threading.Thread):   #inherited from threading.Thread ,overwrite method __init__ and run
	def __init__(self, threadID, name,FileName,DBlist): 
		...
		import_db(self.FILE,self.DB)

def import_db(FILE,DB):
...
if __name__ == '__main__' :
	...
	thread2 = MultiThread(1,"Thread-2",pcw_file,pcw_bak_db)
